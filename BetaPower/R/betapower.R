...
beta_power <- function(mu0, sd0, mu1, sd1, n0, n1, N,a, q, Nsites){
...
mu1_curve_data <- function(mu0, sd0, mu1_min, mu1_max, mu1_step, sd1, n0, n1, N, a, q, Nsites){
  #Get all mu1 values
  ...
  for(i in 1:length(mu1s)){
    ...
    pr <- beta_power(mu0=mu0, sd0=sd0, mu1=mu1s[i], sd1=sd1, n0=n0, n1=n1, N=N, a=a, q=q, Nsites = Nsites)
...
beta_curve <- function(mu0, sd0, mu1_min, mu1_max, mu1_step, sd1, n0s, n1s, N, a, q, Nsites){
  ...
  for(i in 1:length(n1s)){
    ...
    data <- mu1_curve_data(mu0=mu0, sd0=sd0, mu1_min = mu1_min, mu1_max = mu1_max, mu1_step = mu1_step, sd1=sd1, n0=n0, n1=n1, N=N, a=a, q=q, Nsites=Nsites)
    data$n1 <- n1
    data$n0 <- n0
